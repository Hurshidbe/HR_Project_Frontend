...
class ApiService {
  ...
  constructor() {
    ...
    this.api.interceptors.request.use((config) => {
      ...
      console.log(
        "Request interceptor - Token found:",
        !!token,
        "URL:",
        config.url
      );
      if (token) {
        ...
        console.warn(
          "No token found in localStorage for request to:",
          config.url
        );
  ...
  async login(
    data: LoginDto
  ): Promise<CustomBackendResponse<{ status: string; token: string }>> {
    try {
      console.log(
        "Attempting login to:",
        `${this.baseURL}/api/v1/api/v1/users/login`
      );
      ...
      if (
        ...
        throw new Error(
          `Network error: Cannot connect to backend server at ${this.baseURL}. Please ensure the backend is running on port 5000.`
        );
  ...
  async getAllUsers(): Promise<CustomBackendResponse<{ all: User[] }>> {
    ...
    if (user.role !== "superadmin") {
      throw new Error(
        "Access denied: Only superadmin users can view all users"
      );
    }
  ...
  async getUserById(
    id: string
  ): Promise<CustomBackendResponse<{ findOne: User }>> {
    // Check if user is superadmin
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    if (user.role !== "superadmin") {
      throw new Error(
        "Access denied: Only superadmin users can view user details"
      );
    }
  ...
  async createUser(
    data: CreateAdminDto
  ): Promise<CustomBackendResponse<{ admin: User }>> {
    // Check if user is superadmin
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    if (user.role !== "superadmin") {
      throw new Error(
        "Access denied: Only superadmin users can create new users"
      );
    }
  ...
  async updateUser(
    id: string,
    data: LoginDto
  ): Promise<CustomBackendResponse<{ updated: User }>> {
    // Check if user is superadmin
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    if (user.role !== "superadmin") {
  ...
  async deleteUser(
    id: string
  ): Promise<CustomBackendResponse<{ removed: User }>> {
    // Check if user is superadmin
    const user = JSON.parse(localStorage.getItem("user") || "{}");
    if (user.role !== "superadmin") {
  ...
  ): Promise<CustomBackendResponse<Candidate>> {
    try {
      ...
      if (
        ...
        throw new Error(
          "Backend server is not running. Please start the backend server on port 5000 or check the connection."
        );
      } else if (
        ...
        throw new Error(
          "Backend authentication issue: The candidate creation endpoint requires authentication even though it should be public. Please contact the backend administrator to fix this issue."
        );
      } else if (error?.response?.data?.message) {
  ...
  async getAllCandidates(
    ...
    const response = await this.api.get("/api/v1/api/v1/candidates", {
      params: query,
    });
    return response.data;
  }
  ...
  async rejectCandidate(id: string): Promise<CustomBackendResponse<Candidate>> {
    const response = await this.api.patch(
      `/api/v1/api/v1/candidates/${id}/reject`
    );
    return response.data;
  }
  ...
  async acceptCandidate(
    id: string,
    data: AcceptCandidateDto
  ): Promise<CustomBackendResponse<{ accepted: Employee }>> {
    ...
    try {
      const response = await this.api.patch(
        `/api/v1/api/v1/candidates/${id}/accept`,
        data
      );
  ...
  async getCandidateById(
    id: string
  ): Promise<CustomBackendResponse<{ updated: Candidate }>> {
  ...
  async getDepartmentById(
    id: string
  ): Promise<CustomBackendResponse<{ data: Department }>> {
  ...
  async updateDepartment(
    id: string,
    data: CreateDepartmentDto
  ): Promise<CustomBackendResponse<{ updated: Department }>> {
  ...
  async deleteDepartment(
    id: string
  ): Promise<CustomBackendResponse<{ removed: Department }>> {
  ...
  async getPositionsByDepartment(
    departmentId: string
  ): Promise<CustomBackendResponse<{ data: Position[] }>> {
    try {
      ...
      console.log(
        "🔍 getPositionsByDepartment response.data.data:",
        response.data?.data
      );
      ...
      if (response.data?.success) {
        ...
        if (!positionsData || !Array.isArray(positionsData)) {
          ...
          console.error(
            "❌ positionsData isArray:",
            Array.isArray(positionsData)
          );
          throw new Error(
            `Invalid data structure received from API. Expected array, got: ${typeof positionsData}`
          );
        }
        ...
        const filteredPositions = positionsData.filter((position: any) => {
          ...
          const positionDeptId =
            position.departmentId?._id || position.departmentId;
          const matches =
            positionDeptId === departmentId ||
            positionDeptId?.toString() === departmentId;
  ...
  async updatePosition(
    id: string,
    data: CreatePositionDto
  ): Promise<CustomBackendResponse<{ updated: Position }>> {
  ...
  async deletePosition(
    id: string
  ): Promise<CustomBackendResponse<{ deleted: Position }>> {
  ...
  async getAllEmployees(): Promise<
    CustomBackendResponse<{ employees: Employee[] }>
  > {
  ...
  async getEmployeeById(
    id: string
  ): Promise<CustomBackendResponse<{ employee: Employee }>> {
  ...
  async updateEmployeeSalary(
    id: string,
    newSalary: number
  ): Promise<CustomBackendResponse<{ updated: Employee }>> {
    const response = await this.api.patch(`/api/v1/employee/${id}/salary`, {
      newSalary,
    });
    return response.data;
  }
  ...
  async updateEmployeeDepartment(
    ...
    try {
      ...
      throw new Error(
        error.response?.data?.errors?.[0] ||
          "Failed to update employee department"
      );
    }
  }
  ...
  async updateEmployeeStatus(
    ...
    try {
      ...
      const response = await this.api.patch(`/api/v1/employee/${id}/status`, {
        status: newStatus,
      });
  ...
  // History endpoints
  async getSalaryHistory(): Promise<
    CustomBackendResponse<{ salaryHistory: any[] }>
  > {
    const response = await this.api.get("/api/v1/history/salary");
    return response.data;
  }

  async getPositionHistory(): Promise<
    CustomBackendResponse<{ positionHistory: any[] }>
  > {
  ...
  async getSalaryHistoryByEmployee(
    employeeId: string
  ): Promise<CustomBackendResponse<{ salaryHistory: any[] }>> {
    const response = await this.api.get(
      `/api/v1/history/salary/employee/${employeeId}`
    );
    return response.data;
  }

  async getPositionHistoryByEmployee(
    employeeId: string
  ): Promise<CustomBackendResponse<{ positionHistory: any[] }>> {
    const response = await this.api.get(
      `/api/v1/history/position/employee/${employeeId}`
    );
    return response.data;
  }
